...
export const getDiagnosticByIntervention = async (interventionId) => {
  try {
    ...
    // Handle 404 differently - not an error if no diagnostic exists yet
    if (error.response?.status === 404) {
      return { data: null, message: 'Aucun diagnostic trouvé pour cette intervention' };
    }
...
export const createDiagnostic = async (diagnosticData) => {
  try {
    // Convert data format to match what the API expects
