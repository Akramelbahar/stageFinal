...
// API functions - You'll need to create these in src/api/rapports.js
const getAllRapports = async () => {
  // Implement this function to fetch rapports from your backend
  return { data: [] }; // Placeholder
};

const validateRapport = async (id) => {
  // Implement this function to validate a rapport
};

const deleteRapport = async (id) => {
  // Implement this function to delete a rapport
};
...
const Rapports = () => {
  ...
  const getRapportType = (rapport) => {
    if (rapport.renovation_id) return 'Rénovation';
    if (rapport.maintenance_id) return 'Maintenance';
    if (rapport.prestataire_id) return 'Prestataire';
    return 'Autre';
  };

  // Get machine name if available
  const getMachineName = (rapport) => {
    if (rapport.renovation && rapport.renovation.intervention && rapport.renovation.intervention.machine) {
      return rapport.renovation.intervention.machine.nom;
    }
    if (rapport.maintenance && rapport.maintenance.intervention && rapport.maintenance.intervention.machine) {
      return rapport.maintenance.intervention.machine.nom;
    }
    return 'N/A';
  };
  ...
  const filteredRapports = rapports.filter(rapport => {
    ...
    return machineMatch || contentMatch || idMatch;
  });
      ...
      <Card>
        <div className="mb-4 flex flex-col md:flex-row gap-4">
          <div className="relative flex-grow">
            <input
              type="text"
              placeholder="Rechercher par contenu ou machine..."
